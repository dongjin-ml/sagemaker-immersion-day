...
def train_sagemaker(args):
    ...
    if os.environ.get('SM_CURRENT_HOST') is not None:
        args.train_data_path = os.environ.get('SM_CHANNEL_TRAINING')
...
def main():
    ...
    parser.add_argument('--scale_pos_weight', type=int, default=int(os.environ.get("SM_HP_SCALE_POS_WEIGHT", 2)))
    parser.add_argument('--max_depth', type=int, default=int(os.environ.get("SM_HP_MAX_DEPTH", 10)))
    parser.add_argument('--eta', type=float, default=float(os.environ.get("SM_HP_ETA", 0.2)))
    parser.add_argument('--objective', type=str, default=os.environ.get("SM_HP_OBJECTIVE", "binary:logistic"))
    parser.add_argument('--num_round', type=int, default=int(os.environ.get("SM_HP_NUM_ROUND", 999)))
